...
export class CategoryService {
    ...
    getCategories(sort: string = '', order: string = '', page: number = 0, size: number = 0, filters: any = []): Observable<CategoriesWithCount> { 
        ...
        return this.httpClient.get<CategoriesWithCount>(BASE_URL+'/categories?'+params)
            .pipe(catchError(this.handleError));   
    }

    addCategory(category: Category): Observable<string> {
        return this.httpClient.post<string>(BASE_URL+'/categories', category, HTTP_OPTIONS)
            .pipe(catchError(this.handleError));
    }

    updateCategory(category: Category): Observable<string> {
        return this.httpClient.patch<string>(BASE_URL+'/categories/'+category.id, category, HTTP_OPTIONS)
            .pipe(catchError(this.handleError));
    }

    deleteCategories(ids: Array<number>): Observable<string> {            
        return this.httpClient.request<string>('delete', BASE_URL+'/categories', { body: ids })
            .pipe(catchError(this.handleError));
    }
