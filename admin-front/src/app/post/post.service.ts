...
export class PostService {
    ...
    getPost(id: number): Observable<Post> {     
        return this.httpClient.get<Post>(BASE_URL+'/posts/'+id)
    ...
    getPosts(sort: string, order: string, page: number, size: number, filters: any): Observable<PostsWithCount> {
        ...
        return this.httpClient.get<PostsWithCount>(BASE_URL+'/posts?'+params)
            .pipe(catchError(this.handleError));   
    }
    
    addPost(post: Post): Observable<string> {
        return this.httpClient.post<string>(BASE_URL+'/posts', post, HTTP_OPTIONS)
            .pipe(catchError(this.handleError));
    }
    
    updatePost(post: Post): Observable<string> {
        return this.httpClient.patch<string>(BASE_URL+'/posts/'+post.id, post, HTTP_OPTIONS)
            .pipe(catchError(this.handleError));
    }
       
    deletePosts(ids: Array<number>): Observable<string> {            
        return this.httpClient.request<string>('delete', BASE_URL+'/posts', { body: ids })
            .pipe(catchError(this.handleError));
    }
